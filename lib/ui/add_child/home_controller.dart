...
class HomeController extends GetxController {
  ...
  getChildList() async {
    try {
      isLoading(true);
      var value = await CQAPI.getMyChild(id: user.id);
  ...
  void selectEvent(String position) {
    ...
    for (var m in arrBooking[index].arrAllBooking) {
      ...
      for (int i = 0; i < arrList.length; i++) {
        if (m.startDate == arrList[i].startDate &&
...
// page=0&size=20&startDate=01%2F01%2F2010%2000%3A00%3A00%20-0500
  ...
  addReservation(
      Course course, ScheduleModel schedule, String descripton) async {
    if (course == null) {
      ...
      var studentProfileMap = {'id': arrStudent[index.value].id};
